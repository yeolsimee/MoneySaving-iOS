...
//  moneysave
//
//  Created by Mingoo on 2023/04/18.
//

import UIKit
import SnapKit

protocol RoutinCellProtocol {
    func contentAction()
    func checkAction(data: FindRoutineList)
}

class RoutineCell: UITableViewCell {
    var delegate: RoutinCellProtocol?
    
    var data: FindRoutineList?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        btn_Content.addTarget(self, action: #selector(contentAction), for: .touchUpInside)
        btn_Check.addTarget(self, action: #selector(checkAction), for: .touchUpInside)
    }
...
extension RoutineCell {
    ...
    @objc func contentAction() {
        UserInfo.routineId = "\(data!.routineId)"
        delegate?.contentAction()
    }
    
    @objc func checkAction() {
        delegate?.checkAction(data: data!)
    }
}
